...
#Sets parameters for random intervals, to take screenshots whilst the video/ webcam input is running
screenshot_min_interval = 30
screenshot_max_interval = 150
next_screenshot_frame = random.randint(screenshot_min_interval, screenshot_max_interval)
...
while True:
    ...
    for i in range(len(detections)):
        ...
        if conf > 0.5:
            ...
            cv2.putText(frame, label, (xmin, label_ymin-7), cv2.FONT_HERSHEY_DUPPLEX, 0.5, (0, 0, 0), 1) # Draw the label text
    ...
    #Executes the random interval for the screenshot capture for embodied learning
    if frame_index == next_screenshot_frame:
        screenshot_path = os.path.join(screenshot_dir, f'frame_{frame_index:05d}.png')
        cv2.imwrite(screenshot_path, frame)

    #Schedules the next screenshot at random intervals
    next_screenshot_frame = frame_index + random.randint(screenshot_min_interval, screenshot_max_interval)
